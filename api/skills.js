...
function mobileSvg(c) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" role="img" aria-labelledby="title desc"><title id="title">JOEYCH mobile technical focus areas</title><desc id="desc">Mobile-friendly technical categories spanning electromagnetic simulation, hardware design, embedded software, HMI development, computer vision, modelling, and teamwork.</desc><rect width="${W}" height="${H}" fill="${c.bg}"/><path d="M0 .5H360M0 ${H - .5}H360" stroke="${c.border}" stroke-width="1"/><text x="20" y="25" font-family="${FONT}" font-size="10" font-weight="700" letter-spacing="1.5" fill="${c.dim}">// TECHNICAL FOCUS</text><path d="M20 35H340" stroke="${c.border}" stroke-width=".5"/>${rows}<rect width="3" height="${H}" fill="${c.accent}" opacity=".7"/></svg>`;
}
...
export default async function handler(req) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="348" viewBox="0 0 900 348" role="img" aria-labelledby="title desc"><title id="title">JOEYCH technical focus areas</title><desc id="desc">Technical categories spanning electromagnetic simulation, hardware design, embedded software, HMI development, computer vision, modelling, and teamwork.</desc><rect width="900" height="348" fill="${c.bg}"/><path d="M0 .5H900M0 347.5H900" stroke="${c.border}" stroke-width="1"/><text x="24" y="16" font-family="${FONT}" font-size="9" font-weight="700" letter-spacing="2" fill="${c.dim}">// TECHNICAL FOCUS</text><path d="M24 24H876" stroke="${c.border}" stroke-width=".5"/><text x="42" y="40" font-family="${FONT}" font-size="9" letter-spacing="1" fill="${c.dim}">CATEGORY</text><text x="240" y="40" font-family="${FONT}" font-size="9" letter-spacing="1" fill="${c.dim}">FACTUAL FOCUS</text>${rows}<rect width="3" height="348" fill="${c.accent}" opacity=".7"/></svg>`;
