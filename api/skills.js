...
function mobileSvg(c) {
  ...
  const rows = FOCUS.map(([label, lineOne, lineTwo], index) => {
    const y = 58 + index * 50;
    return `<circle cx="24" cy="${y - 5}" r="3.5" fill="${index < 6 ? c.accent : c.info}"/><text x="38" y="${y}" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.text}">${label}</text><text x="38" y="${y + 16}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineOne}</text><text x="38" y="${y + 30}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineTwo}</text><path d="M20 ${y + 40}H340" stroke="${c.border}" stroke-width=".5"/>`;
  }).join("");
...
export default async function handler(req) {
  ...
  const rows = FOCUS.map(([label, lineOne, lineTwo], index) => {
    const y = 55 + index * 36;
    return `<circle cx="28" cy="${y - 4}" r="3" fill="${index < 6 ? c.accent : c.info}"/><text x="42" y="${y}" font-family="${FONT}" font-size="12" font-weight="700" fill="${c.text}">${label}</text><text x="240" y="${y}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineOne} ${lineTwo}</text><path d="M24 ${y + 14.5}H876" stroke="${c.border}" stroke-width=".5"/>`;
  }).join("");
