...
function mobileSvg(c) {
  ...
  const rows = FOCUS.map(([label, lineOne, lineTwo], index) => {
    const y = 58 + index * 50;
    return `<circle cx="24" cy="${y - 5}" r="3.5" fill="${c.accent}"/><text x="38" y="${y}" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.text}">${label}</text><text x="38" y="${y + 16}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineOne}</text><text x="38" y="${y + 30}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineTwo}</text><path d="M20 ${y + 40}H340" stroke="${c.border}" stroke-width=".5"/>`;
  }).join("");
...
export default async function handler(req) {
  ...
  const rows = FOCUS.map(([label, lineOne, lineTwo], index) => {
    const y = 55 + index * 36;
    return `<circle cx="28" cy="${y - 4}" r="3" fill="${c.accent}"/><text x="42" y="${y}" font-family="${FONT}" font-size="12" font-weight="700" fill="${c.text}">${label}</text><text x="240" y="${y}" font-family="${FONT}" font-size="11" fill="${c.muted}">${lineOne} ${lineTwo}</text><path d="M24 ${y + 14.5}H876" stroke="${c.border}" stroke-width=".5"/>`;
  }).join("");
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="348" viewBox="0 0 900 348" role="img" aria-labelledby="title desc"><title id="title">JOEYCH technical focus areas</title><desc id="desc">Technical categories spanning embedded firmware, PCB and EDA, flight control, agent engineering, local AI, interface craft, electromagnetic simulation, and HMI development.</desc><rect width="900" height="348" fill="${c.bg}"/><path d="M0 .5H900M0 347.5H900" stroke="${c.border}" stroke-width="1"/><text x="24" y="16" font-family="${FONT}" font-size="9" font-weight="700" letter-spacing="2" fill="${c.dim}">// TECHNICAL FOCUS</text><path d="M24 24H876" stroke="${c.border}" stroke-width=".5"/>${rows}<rect width="3" height="348" fill="${c.accent}" opacity=".7"/></svg>`;
