...
function mobileSvg(c) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="360" height="184" viewBox="0 0 360 184" role="img" aria-labelledby="title desc"><title id="title">JOEYCH mobile verified links</title><desc id="desc">GitHub, personal website, and original eFlyDrone-Boards project links.</desc><style>a:hover rect,a:focus rect{stroke:${c.accent};stroke-width:1}</style><defs><clipPath id="clip"><rect width="360" height="184" rx="8"/></clipPath></defs><g clip-path="url(#clip)"><rect width="360" height="184" rx="8" fill="${c.bg}"/><path d="M0 .5H360M359.5 0V184M0 183.5H360" stroke="${c.border}" stroke-width="1" fill="none"/>${pills}<rect width="3" height="184" fill="${c.accent}" opacity=".7"/></g></svg>`;
}
...
export default async function handler(req) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="92" viewBox="0 0 900 92" role="img" aria-labelledby="title desc"><title id="title">JOEYCH verified links</title><desc id="desc">GitHub, personal website, and original eFlyDrone-Boards project links.</desc><style>a:hover rect,a:focus rect{stroke:${c.accent};stroke-width:1}</style><defs><clipPath id="clip"><rect width="900" height="92" rx="8"/></clipPath></defs><g clip-path="url(#clip)"><rect width="900" height="92" fill="${c.bg}"/><path d="M0 .5H900M899.5 0V92M0 91.5H900" stroke="${c.border}" stroke-width="1" fill="none"/>${pills}<text x="872" y="50" text-anchor="end" font-family="${FONT}" font-size="12" font-weight="800" fill="${c.text}">JOEYCH</text><rect width="3" height="92" fill="${c.accent}" opacity=".7"/></g></svg>`;
