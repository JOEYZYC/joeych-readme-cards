...
function mobileSvg(c) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="360" height="184" viewBox="0 0 360 184" role="img" aria-labelledby="title desc"><title id="title">JOEYCH 的移动端已验证链接</title><desc id="desc">GitHub、个人网站和 eFlyDrone-Boards 原创项目链接。</desc><defs><linearGradient id="bar" x1="0" x2="1"><stop stop-color="${c.bar}"/><stop offset="1" stop-color="${c.bg}"/></linearGradient></defs><style>a:hover rect,a:focus rect{stroke:${c.accent};stroke-width:1}</style><rect width="360" height="184" rx="8" fill="${c.bg}"/><rect width="360" height="34" fill="url(#bar)"/><path d="M0 34.5H360M0 .5H360M359.5 0V184M0 183.5H360" stroke="${c.border}" stroke-width="1" fill="none"/><circle cx="20" cy="17" r="4" fill="#ff5f57"/><circle cx="34" cy="17" r="4" fill="#febc2e"/><circle cx="48" cy="17" r="4" fill="#28c840"/><text x="64" y="21" font-family="${FONT}" font-size="10" font-weight="600" fill="${c.dim}">connect · document · build</text>${pills}<rect width="3" height="184" fill="${c.accent}" opacity=".7"/></svg>`;
}
...
export default async function handler(req) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="92" viewBox="0 0 900 92" role="img" aria-labelledby="title desc"><title id="title">JOEYCH 的已验证链接</title><desc id="desc">GitHub、个人网站和 eFlyDrone-Boards 原创项目链接。</desc><defs><linearGradient id="bar" x1="0" x2="1"><stop stop-color="${c.bar}"/><stop offset="1" stop-color="${c.bg}"/></linearGradient></defs><style>a:hover rect,a:focus rect{stroke:${c.accent};stroke-width:1}</style><rect width="900" height="92" fill="${c.bg}"/><rect width="900" height="34" fill="url(#bar)"/><path d="M0 34.5H900M0 .5H900M899.5 0V92M0 91.5H900" stroke="${c.border}" stroke-width="1" fill="none"/><circle cx="22" cy="17" r="4.5" fill="#ff5f57"/><circle cx="38" cy="17" r="4.5" fill="#febc2e"/><circle cx="54" cy="17" r="4.5" fill="#28c840"/><text x="74" y="21" font-family="${FONT}" font-size="11" font-weight="600" fill="${c.dim}" letter-spacing=".8">connect · document · build</text>${pills}<text x="872" y="69" text-anchor="end" font-family="${FONT}" font-size="12" font-weight="800" fill="${c.text}">JOEYCH</text><rect width="3" height="92" fill="${c.accent}" opacity=".7"/></svg>`;
