...
function mobileSvg(c) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="360" height="72" viewBox="0 0 360 72" role="img" aria-labelledby="title desc"><title id="title">JOEYCH terminal card service</title><desc id="desc">View the JOEYZYC-maintained terminal-style GitHub README card source and documentation.</desc><defs><clipPath id="clip"><rect width="360" height="72" rx="8"/></clipPath></defs><g clip-path="url(#clip)"><rect width="360" height="72" fill="${c.bg}"/><path d="M0 .5H360M0 71.5H360" stroke="${c.border}" stroke-width="1"/><rect x="16" y="10" width="328" height="52" rx="4" fill="${c.panel}" stroke="${c.border}" stroke-width=".5"/><rect width="3" height="72" fill="${c.accent}" opacity=".7"/><text x="30" y="32" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.accent}">&gt;_</text><text x="54" y="32" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.text}">joeych-readme-cards</text><text x="54" y="51" font-family="${FONT}" font-size="12" font-weight="700" fill="${c.dim}">source &amp; docs</text></g></svg>`;
}
