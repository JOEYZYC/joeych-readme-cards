...
function mobileSvg(c) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="360" height="72" viewBox="0 0 360 72" role="img" aria-labelledby="title desc"><title id="title">本地终端卡片服务</title><desc id="desc">查看 JOEYZYC 本地维护的终端式 GitHub README 卡片源码与说明。</desc><rect width="360" height="72" rx="6" fill="${c.bg}"/><path d="M0 .5H360M0 71.5H360" stroke="${c.border}" stroke-width="1"/><rect x="16" y="10" width="328" height="52" rx="4" fill="${c.panel}" stroke="${c.border}" stroke-width=".5"/><rect width="3" height="72" fill="${c.accent}" opacity=".7"/><text x="30" y="32" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.accent}">&gt;_</text><text x="54" y="32" font-family="${FONT}" font-size="13" font-weight="700" fill="${c.text}">查看本地终端卡片服务</text><text x="54" y="51" font-family="${FONT}" font-size="11" font-weight="700" fill="${c.dim}">[ SOURCE ]</text></svg>`;
}
...
export default async function handler(req) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="44" viewBox="0 0 900 44" role="img" aria-labelledby="title desc"><title id="title">本地终端卡片服务</title><desc id="desc">查看 JOEYZYC 本地维护的终端式 GitHub README 卡片源码与说明。</desc><rect width="900" height="44" fill="${c.bg}"/><path d="M0 .5H900M0 43.5H900" stroke="${c.border}" stroke-width="1"/><rect x="20" y="7" width="860" height="30" rx="4" fill="${c.panel}" stroke="${c.border}" stroke-width=".5"/><rect width="3" height="44" fill="${c.accent}" opacity=".7"/><text x="35" y="26" font-family="${FONT}" font-size="12" font-weight="700" fill="${c.accent}">&gt;_</text><text x="450" y="26" text-anchor="middle" font-family="${FONT}" font-size="12" font-weight="700" letter-spacing="1" fill="${c.text}">查看本地终端卡片服务说明</text><text x="865" y="26" text-anchor="end" font-family="${FONT}" font-size="11" font-weight="700" fill="${c.dim}">[ SOURCE ]</text></svg>`;
